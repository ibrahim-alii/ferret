...
async def test_expand_node_fetches_parent_sections_for_reranked_children(monkeypatch):
    ...
    mock_row = {"chunk_id": "p1", "parent_chunk_id": None, "paper_id": "arxiv1", "section_title": "Intro", "text": "Parent text"}
...
async def test_expand_node_caps_distinct_parents(monkeypatch):
    ...
    async def fake_fetchone():
        return {"chunk_id": "px", "parent_chunk_id": None, "paper_id": "arxiv1", "section_title": "S", "text": "T"}

    call_count = 0

    async def fake_execute(sql, params=None):
        mock_cursor = AsyncMock()
        mock_cursor.fetchone = AsyncMock(return_value={"chunk_id": params[0] if params else "px", "parent_chunk_id": None, "paper_id": "arxiv1", "section_title": "S", "text": f"text_{call_count}"})
        return mock_cursor
...
async def test_astream_chat_yields_token_events_then_done_event(monkeypatch):
    ...
    mock_cursor = AsyncMock()
    mock_cursor.fetchone = AsyncMock(return_value={"chunk_id": "p1", "parent_chunk_id": None, "paper_id": "arxiv1", "section_title": "S", "text": "context"})
    mock_conn = AsyncMock()
    mock_conn.execute = AsyncMock(return_value=mock_cursor)
    mock_conn.__aenter__ = AsyncMock(return_value=mock_conn)
    mock_conn.__aexit__ = AsyncMock(return_value=False)
